...
export const getKingByIdService = async (kingId, userId) => {
  const [king, collections] = await Promise.all([
    findKingById(kingId),
  ...
  const isCollected = collections.some((c) => c.kingId === kingId);
  return { ...king, imageUrl: getKingImageUrl(king), tags, isCollected };
};
